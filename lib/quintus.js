...
var Quintus = function Quintus(opts) {
  ...
  */
  Q.assetUrl = function(base,url) {
    ...
    if(Q._isHttp(url) || Q._isAbsolute(url)) {
      return url + timQ.assetUrlestamp;
    } else {
  ...
  /**
   Helper method to test if a url uses the http(s)
   protocol

   @for Quintus
   @method _isHttp
   @param {String} url
   @return {Boolean} true when the url starts with http(s)
  */
  Q._isHttp = function(url){
      return /^https?:\/\//.test(url);
  }

  /**
   Helper method to test if a url is absolute

   @for Quintus
   @method _isAbsolute
   @param {String} url
   @return {Boolean} true when the url starts with /
  */
  Q._isAbsolute = function(url){
      return url[0] === "/";
  }

  /**
   Helper method to test if the protocol of a url refers 
   to the local file system

   @for Quintus
   @method _isFileURL
   @param {String} url
   @return {Boolean} true when the url starts with file:
  */
  Q._isFile = function(url) {
  ...
  */
  ...
  Q._JSONFileURLMessage =  "You are trying load .json from the local file system.";
  /**
   Helper method to test file name has an extension

   @for Quintus
   @method _hasExtension
   @param {String} filename
   @param {String} extension (withouth the dot)
   @return {Boolean} true when the filenames ends with the extension
  */
  Q._hasExtension = function(filename,extension) {
    return Q._fileExtension(filename) === extension;
  };

  /**
   Helper method to test if the request is for a 
   json file on the local file system.

   @for Quintus
   @method _isFileURL
   @param {String} url
   @return {Boolean} true when the path starts 
      with file: and the file ends with json
  */
  Q._isJSONFileURL = function(path,file) {
    if(!Q._hasExtension(src,'json')){
      return false;
    }

    var assetUrl = Q.assetUrl(Q.options.dataPath,src);
    if(Q._isHttp(assetUrl)){
      return false;
    }
  
    return Q._isFile(window.location.href)
  };
